...
async function startSlot(slot) {
  ...
  const sock = makeWASocket({
    ...
    // IMPORTANTE:
    emitOwnEvents: true,          // asegura que también recibas tus mensajes salientes
    syncFullHistory: false        // no backfill automático (puedes poner true si quieres probar)
  });
  ...
  // Helper robusto para extraer texto “humano”
  ...
  // Para grupos, a veces el número está en participant
  const toTel = (jid) => (jid||'').replace(/@.*/, '');

  sock.ev.on('messages.upsert', ({ messages }) => {
    log.info({ slot, count: messages.length }, 'Mensajes recibidos');
    for (const m of messages) {
      const remoteJid = m?.key?.remoteJid;
      if (!remoteJid) continue;

      const fromMe = !!m?.key?.fromMe;
      const participant = m?.key?.participant; // grupos
      ...
      const row = {
        ...
        jid: remoteJid,
        from_number: tel,
        ts: (m.messageTimestamp || m.timestamp || Math.flo
